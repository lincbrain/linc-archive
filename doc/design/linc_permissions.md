...
- Update CloudFront cache policy time to live (TTL) setting to 1 second.  Since LINC assets can be overwritten with the same filename, the CloudFront cache layer would not know to update the cache contents.  This updated policy will allow for the updated asset to be displayed correctly by Neuroglancer.
...
See discussion with Neuroglancer team in [Issue 507](https://github.com/google/neuroglancer/issues/507).
